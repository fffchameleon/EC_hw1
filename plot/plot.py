...
folder_path = 'data'
output_file = 'show/uniform.png'
filename_pattern = 'avgfitness_uniform'
...
for file in csv_files:
    ...
    # var = int(file.split('_')[-1].split('.')[0] + '.' + file.split('_')[-1].split('.')[1])
    var = int(file.split('_')[-1].split('.')[0])

    plt.plot(data['Binary'], label=f'binary_{var}')
    plt.plot(data['Real'], label=f'real_{var}')

plt.title('Independent variable: uniform crossover or not')
